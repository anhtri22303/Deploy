...
const Buy = () => {
  ...
  const CustomerSchema = Yup.object().shape({
    ...
    mobile: Yup.string()
      ...
      .matches(/^[^\s].*[^\s]$/, 'Mobile cannot have leading or trailing spaces')
      .length(10, 'Mobile must be exactly 10 characters'),
    email: Yup.string()
      .email('Invalid email')
      .required('Email is required')
      .matches(/^[^\s].*[^\s]$/, 'Email cannot have leading or trailing spaces'),
  });
  ...
  const validate = (fieldName) => {
    ...
    if (!fieldName || fieldName === "Name") tempErrors.Name = Name && !Name.startsWith(' ') ? "" : "This field is required and cannot start with a space.";
    if (!fieldName || fieldName === "Type") tempErrors.Type = type ? "" : "This field is required.";
    if (!fieldName || fieldName === "goldWeight") tempErrors.goldWeight = goldWeight && !goldWeight.toString().startsWith(' ') ? "" : "This field is required and cannot start with a space.";
    if (!fieldName || fieldName === "diamondWeight") tempErrors.diamondWeight = diamondWeight && !diamondWeight.toString().startsWith(' ') ? "" : "This field is required and cannot start with a space.";
    
    if (!fieldName || fieldName === "component1") tempErrors.component1 = components && components[0] ? "" : "Component 1 is required.";
    if (!fieldName || fieldName === "component2") tempErrors.component2 = components && components[1] ? "" : "Component 2 is required.";

    if (!fieldName || fieldName === "code") tempErrors.code = code.trim() ? "" : "Code is required and cannot be empty or start with a space.";
      ...
      <form onSubmit={handleSubmit} style={{ width: "100%" }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            <FormControl fullWidth margin="normal" error={Boolean(errors.component1)}>
              ...
              {errors.component1 && <FormHelperText>{errors.component1}</FormHelperText>}
            </FormControl>
          </Grid>
          <Grid item xs={12} sm={6}>
            <FormControl fullWidth margin="normal" error={Boolean(errors.component2)}>
              ...
              {errors.component2 && <FormHelperText>{errors.component2}</FormHelperText>}
            </FormControl>
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField
          ...
          onSubmit={async (values, actions) => {
            try {
              ...
              if (error.response && error.response.data && error.response.data.message) {
                ...
                toast.error("Failed to create buyback. Please try again.");
              }
              console.error("error:", error);
              actions.setSubmitting(false);
            }
